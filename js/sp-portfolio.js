...
window.Portfolio = {
  ...
  loadData: async function() {
    ...
    this.deals = rawDeals.map(d => {
      ...
      noi: d.noi || (hasExplicitValuation ? 0 : Math.round((d.raise || 0) * 0.08)),
      expenses: d.expenses || (hasExplicitValuation ? 0 : Math.round((d.raise || 0) * 0.03)),
      debtService: d.debtService || (hasExplicitValuation ? 0 : Math.round((d.raise || 0) * 0.04)),
      irr: d.irr || 0,
      moic: d.moic || (hasExplicitValuation && (d.raise || 0) > 0 ? +(d.currentValue / d.raise).toFixed(2) : 0),
      occupancy: d.occupancy || (d.status === 'operating' ? 93 : 0),
  ...
  generateDemoInvestors: function() {
    // Read from Firestore via SP — no hardcoded demo data
